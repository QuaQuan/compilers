...
setwd("F:/experience/f7v23")
data <- read.csv("./FormatResult_v23.csv",header=FALSE)


#----------将原始数据根据class划分为0/1——————----------------------------——————#
#-----每个程序需要根据函数数量改变—————————#
colnames(data) <- c("a1","a2","a3","a4","a5","a6","a7","class")
...
#--------------7个程序不同，需要修改----------------#
colnames(para) <- c("a1","a2","a3","a4","a5","a6","a7","nnet","function")
write.csv(para,file="./testData.csv",row.names = FALSE)
colnames(temp2) <- c("a1","a2","a3","a4","a5","a6","a7","class")
